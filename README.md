...
-   ... Actual application might look slightly different as per design.
-   ... Button icons (show, hide, location) was missing in provided assests, so it will not not be visible, but if button component `options` includes `icon` property then it will show the icon on expected location.
-   ... Actual application behaviour might not match exactly as mock up outof the box, As I have modified page data to test multiple scenarios.
